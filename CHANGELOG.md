...
- **BREAKING:** The generated single-model `(*Foo).Update` and `(*Foo).Reload` now copy **only the column fields** out of the row they fetched, instead of overwriting the whole receiver with `*o = *v`. Every non-column field on the model — the relationship cache `.R` (including `R.Loaded`), the counts plugin's `.C`, and anything a plugin adds through the `model/fields/additional` template block — is now left untouched.

  Previously both methods assigned the freshly scanned model over the receiver. That model comes from an `UPDATE ... RETURNING` (or a plain `SELECT` for `Reload`), which never loads relationships or counts, so all of its non-column fields are zero values. The net effect was that a successful `Update()` or `Reload()` silently emptied `.R`, reset every `R.Loaded.<Rel>` flag to `false`, and cleared every count pointer in `.C`. `Reload` did carry `.R` forward explicitly (`o2.R = o.R`), but nothing carried `.C` or any other plugin field, on any dialect.

  Copying only the columns fixes this class of bug once instead of once per field: the set of columns is fully known at generation time, while the set of non-column fields is open-ended (plugins can add more through `model/fields/additional`), so an approach that enumerates what to *preserve* can never be complete, while enumerating what to *copy* can. All columns are copied, including generated ones, so values computed by the database (generated columns, trigger results, `DEFAULT`s) are still reflected exactly as before.

  The MySQL dialect already worked this way: its `one_update` override calls `s.Overwrite(o)` and never touches non-column fields. This change brings the default block (PostgreSQL, SQLite) in line for `Update`, and — since `Reload` has no dialect override — fixes `Reload` on every dialect.

  **Breaking change:** any code relying on `Update()` or `Reload()` clearing `.R` — most notably a "reload if not loaded" pattern keyed off `R.Loaded.<Rel>` — will stop reloading, because `R.Loaded.<Rel>` now stays `true` and keeps pointing at pre-update data. The same applies to `.C`: counts loaded before the call now survive it, and are no longer implicitly invalidated. `.R` and `.C` have always been caches with no freshness guarantee; callers that need post-update relationship data or counts should fetch it explicitly (`Reload` for columns, `Load<Rel>` / `Preload` / `LoadCount<Rel>` / `PreloadCount` for the caches) rather than relying on these methods to clear the caches as a side effect.
