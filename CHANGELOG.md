...
- **BREAKING:** The generated `<Table>Slice.copyMatchingRows` — the helper behind `UpdateAll`, `ReloadAll` and the `UpdateMod`/`DeleteMod`/`MergeMod` loaders — now copies **only the column fields** from the freshly fetched row onto the model already in the slice, instead of carrying `.R` over to the new model and then swapping the slice element to point at it. Every non-column field on the model — the relationship cache `.R` (including `R.Loaded`), the counts plugin's `.C`, and anything a plugin adds through the `model/fields/additional` template block — is now left untouched, and the slice keeps its original pointers.

  Previously only `.R` was carried across (`new.R = old.R`), so `.C` and every other plugin-added field was silently reset to its zero value by `UpdateAll` (PostgreSQL and SQLite; MySQL overrides `slice_update` and was never affected), and by `ReloadAll` (every dialect — `ReloadAll` has no dialect override), as well as by `MergeMod` when the caller runs the merge with `.All()` (PostgreSQL only). Enumerating the fields to preserve cannot be complete, since plugins can add more; copying only the columns fixes the whole class at once, because the set of columns is fully known at generation time.

  MySQL's `slice_update` override already worked this way — it calls `vals.Overwrite(o[i])` on the models already in the slice and never swaps pointers — so this brings the default path in line with the dialect that already had it right.

  **Breaking change:** the models in the slice are no longer replaced with the freshly scanned ones. Code that relied on the pointer identity changing (for example, comparing a previously held `*Foo` against `slice[i]` to detect a refresh) will observe the two pointers as equal now. Conversely — and this is the intended improvement — a caller holding a pointer to a member of the slice now *does* see the new column values after `UpdateAll`/`ReloadAll`, matching how the single-model `Update`/`Reload` behave. Additionally, the freshly scanned models are no longer mutated: previously `copyMatchingRows` wrote the old model's `.R` onto them. Callers that run `Table.Update(vals.UpdateMod(), slice.UpdateMod()).All(...)` (or the `MergeMod` equivalent) themselves and read `.R` off the returned slice will now find it empty; the loaded relationships stay on the models in the original slice instead.
